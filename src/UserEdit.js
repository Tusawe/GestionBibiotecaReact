...
class UserEdit extends Component {
  ...
  async componentDidMount() {
    if (this.props.match.params.ins !== 'new') {
      const user = await (await fetch(`/api/usuario/${this.props.match.params.ins}`)).json();
  ...
  render() {
    const {item} = this.state;
    const title = <h2>{item.nombre ? 'Editar Usuario' : 'Añadir Usuario'}</h2>;
